...
protocol Feature1Repository {
    var recordsPublisher: AnyPublisher<Result<[CalendarInfo], Feature1ServiceError>, Never> { get }
    func loadRecords() throws -> [CalendarInfo]
    func saveRecord(_ info: [CalendarInfo]) throws
}

final class KeyValueRecordsRepository: Feature1Repository {
    private let storage: DefaultsStorage
    
    private let recordSubject = PassthroughSubject<Result<[CalendarInfo], Feature1ServiceError>, Never>()
    
    var recordsPublisher: AnyPublisher<Result<[CalendarInfo], Feature1ServiceError>, Never> {
...
actor Feature1Service {
    nonisolated var recordsPublisher: AnyPublisher<Result<[CalendarInfo], Feature1ServiceError>, Never> { recordsSubject.eraseToAnyPublisher() }
    
    private var records: [CalendarInfo] = []
    private var cancellables = Set<AnyCancellable>()
    private let repository: Feature1Repository
    private let recordsSubject = CurrentValueSubject<Result<[CalendarInfo], Feature1ServiceError>, Never>(.success([]))
    ...
    private func updateRecordsSubject(_ result: Result<[CalendarInfo], Feature1ServiceError>) {
    ...
    func saveRecord(_ result: CalendarInfo) async {
        do {
